...
class GenericContainer(ContainerBuilder):
    ...
    def with_wait_for_http(self, http_port: str, status_code: int=200, end_point: str="/", server_startup_time: int=20) -> bool:
        response_check: bool = True
        for _ in range(0, 3):
            sleep(server_startup_time)
            try:
                site_url: str = f"http://{self.get_container_host_ip()}:{self.get_exposed_port(port=http_port)}/{end_point.lstrip('/')}"
