...
def produce_kafka_msg(runner, app_env_vars, version):
    ...
    producer = Producer({
        "bootstrap.servers": f"{kafka_env_vars.get('DOCKER_PYTHON_EXTERNAL_HOST')}:{kafka_mapped_port}",
        "debug": "all",
        "socket.timeout.ms": 60
    })
    producer.poll(0)
    producer.produce(f"{app_env_vars.get('KAFKA_TOPIC')}", json.dumps(json.dumps(json.loads(version))).encode('utf-8'))
