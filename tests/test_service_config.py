...
def test_get_config_services_more_containers(valid_container_config):
    ...
    assert len(provided_service_config.services) == len(computed_services_instance.ranked_itest_config_services.services)

    assert computed_services_instance.ranked_itest_config_services.network.name == NetworkConstants.DEFAULT_NETWORK_MODE
    ...
    with pytest.raises(AttributeError):
        ContainerNetwork(
            docker_client=from_env(),
            network_name="random_non_existing_network"
        )
